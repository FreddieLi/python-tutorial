...
"""
# This answer is from chatGPT.  BFS
...
class RobotGrid:
    ...
    def is_valid(self, x, y):
        return 0 <= x <= self.grid_size[0] and 0 <= y <= self.grid_size[1] and (x, y) not in self.occupied_cells
        ...
        while queue:
            ...
            for move in moves:
                if self.is_valid(*move):
                    queue.append((move, path + [current])) #append tuple(move) and list(path + [current])
            # print(cnt,end="")
            # print(queue)
            cnt+=1
